@component('forum::modal-form')
    ...
    @if($category->parent_id  != null && $privateAncestor == null)
        <div class="mb-3">
            <label for="parent_id">Parent</label>
                <select class="form-select" id="parent_id" name="parent_id" required >
                        @foreach($parent_cat as $p_cat)
                            @if($p_cat->id == $category->parent_id )
                                <option value="NULL" disabled selected>{{$p_cat->title}}</option>
                            @else
    ...
    <div class="mb-3">
        ...
        <input type="hidden" name="pinned" value="0" />
            <input class="form-check-input" type="checkbox" name="pinned" id="pinned" value="1" {{ $category->pinned ? 'checked' : '' }}>
